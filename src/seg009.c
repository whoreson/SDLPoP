...
void __pascal far restore_stuff() {
	if (pop_joystick != NULL) {
		SDL_JoystickClose(pop_joystick);
		pop_joystick = NULL;
	}
	SDL_Quit();
}
...
#ifdef __IPHONEOS__
	/* Accelerometer-based quit: detect phone in docker or face-down.
	 *
	 * SDL_JoystickGetAxis() returns REMAPPED values in landscape mode:
	 *   js X = -Device Y, js Y = -Device X, js Z = Device Z
	 *
	 * Docker (device: X=+0.02, Y=-0.43, Z=-0.15):
	 *   js X = -(-0.43)*65534 = +28172  (strong positive)
	 *   js Y = -(+0.02)*65534 = -1534   (near zero)
	 *   js Z = (-0.15)*65534 = -9726    (moderate negative)
	 *
	 * Face-down (device: X~0, Y~0, Z=+0.38):
	 *   js X = ~0
	 *   js Y = ~0
	 *   js Z = +0.38*65534 = +24903    (strong positive) */
	{
	 extern SDL_Joystick *pop_joystick;

	 if (pop_joystick) {
	  Sint16 xaxis = SDL_JoystickGetAxis(pop_joystick, 0); // ingame left-right
	  Sint16 yaxis = SDL_JoystickGetAxis(pop_joystick, 1); // ingame up-down
	  Sint16 zaxis = SDL_JoystickGetAxis(pop_joystick, 2);
	  Sint16 abs_y = yaxis < 0 ? -yaxis : yaxis;
	  Sint16 abs_z = zaxis < 0 ? -zaxis : zaxis;

	  /* iPhone is in the cradle */
	  if (xaxis > 27000 && abs_y < 3000 && abs_z < 12000) {
		quit(0);
	  }
	  /* Face-down: js Z strongly positive (>16384), js X and js Y near zero */
	  else if (zaxis > 16384 && xaxis > -16384 && xaxis < 16384 && abs_y < 16384) {
	   quit(0);
	  }
	 }
	}
...
dat_type *__pascal open_dat(const char *filename,int drive) {
	...
	struct stat st;
	if (fp && fstat(fileno(fp), &st) == 0 && S_ISDIR(st.st_mode)) {
		fclose(fp); fp = NULL;
	}
...
chtab_type* __pascal load_sprites_from_file(int resource,int palette_bits/*,byte* pack,int shift*/, int quit_on_error) {
	...
	/* Read raw .pal bytes and extract fields at packed offsets. */
	/* GCC 4.0.1 LLVM ignores #pragma pack, so dat_shpl_type is 120 bytes not 100. */
	/* Packed layout: n_images@0, row_bits@1(2B), n_colors@3, vga[16]@4 (3B stride) */
	byte* raw_pal = (byte*) load_from_opendats_alloc(resource, "pal", NULL, NULL);
	if (raw_pal == NULL) {
	...
	/* Build a dat_pal_type with correct palette values. */
	static dat_pal_type g_pal_buffer;
	g_pal_buffer.row_bits = (word)(raw_pal[1] | (raw_pal[2] << 8));
	g_pal_buffer.n_colors = raw_pal[3];
	for (i = 0; i < 16; ++i) {
		g_pal_buffer.vga[i].r = raw_pal[4 + i*3 + 0];
		g_pal_buffer.vga[i].g = raw_pal[4 + i*3 + 1];
		g_pal_buffer.vga[i].b = raw_pal[4 + i*3 + 2];
	}
	dat_pal_type* pal_ptr = &g_pal_buffer;
	n_images = raw_pal[0];
...
int __pascal far set_joy_mode() {
	if (joystick_available && pop_joystick == NULL) {
		(pop_joystick = SDL_JoystickOpen(0));
	}
	if (pop_joystick != NULL) {
		is_joyst_mode = 1;
		is_keyboard_mode = 0;
...
// seg009:7640
extern int digi_unavailable;
void __pascal far play_speaker_sound(sound_buffer_type far *buffer) {
	int time_ms;
	// Calculate duration before digi_unavailable check
	// Use raw byte access because note_type is 3 bytes but compiler may pad it
	// note_type layout: word frequency (2 bytes LE) + byte length (1 byte)
	int length = 0;
	{
		const byte* raw = (const byte*)buffer->data.speaker.notes;
		int offset = 0;
		while (1) {
			word freq = raw[offset] | (raw[offset+1] << 8);
			byte len = raw[offset+2];
			if (freq == 0x12) break;
			length += len;
			offset += 3;
		}
	...
	{
		const byte* raw_buf = (const byte*)&buffer->data.speaker.tempo;
		word tempo_raw = raw_buf[0] | (raw_buf[1] << 8);
	}
	// stub
	if (digi_unavailable) return;
	{
		const byte* raw_buf2 = (const byte*)&buffer->data.speaker.tempo;
		word tempo_raw2 = raw_buf2[0] | (raw_buf2[1] << 8);
		time_ms = length*1000 / tempo_raw2;
	}
...
void init_digi() {
	...
	desired->freq = digi_samplerate; //buffer->data.digi.sample_rate;
	desired->format = AUDIO_U8;
	desired->channels = 1;
	//desired->samples = buffer->data.digi.sample_count;
	...
	/* Initialize MIDI playback and register as music hook */
#ifdef __IPHONEOS__MIDI
	init_midi();
	Mix_HookMusic(midi_callback, NULL);
#endif
#endif
...
void __pascal far play_chunk_sound(sound_buffer_type far *buffer) {
	...
	if (Mix_PlayChannel(sound_channel, buffer->data.chunk, 0) == -1) {
...
void __pascal far play_digi_sound(sound_buffer_type far *buffer) {
	...
	if (buffer->data.digi.sample_size != 8) return;
#ifndef USE_MIXER	
	SDL_AudioCVT cvt;
	memset(&cvt, 0, sizeof(cvt));
	int result = SDL_BuildAudioCVT(&cvt,
		AUDIO_U8, 1, buffer->data.digi.sample_rate,
	...
	int dlen = buffer->data.digi.sample_count; // if format is AUDIO_U8
	cvt.buf = (Uint8*) malloc(dlen * cvt.len_mult);
	memcpy(cvt.buf, buffer->data.digi.samples, dlen);
	cvt.len = dlen;
	...
	digi_buffer = cvt.buf;
	digi_playing = 1;
//	digi_remaining_length = buffer->data.digi.sample_count;
//	digi_remaining_pos = buffer->data.digi.samples;
...
//#error "What to put here?..."
	int size = buffer->data.digi.sample_count;
	...
	wav_data->SampleRate = buffer->data.digi.sample_rate;
	wav_data->BitsPerSample = buffer->data.digi.sample_size;
	...
	memcpy(wav_data->Data, buffer->data.digi.samples, size);
	SDL_RWops* rw = SDL_RWFromConstMem(wav_data, alloc_size);
	Mix_Chunk *chunk = Mix_LoadWAV_RW(rw, 1);
	if (chunk == NULL) {
		fprintf(stdout, "Converting to dump.wav\n");
		FILE* fp = fopen("/var/tmp/dump.wav","wb");
...
void free_sound(sound_buffer_type far *buffer) {
	...
	if (buffer->type == sound_chunk) {
		Mix_FreeChunk(buffer->data.chunk);
	}
#endif
	free(buffer);
}
...
void __pascal far set_gr_mode(byte grmode) {
	...
	SDL_EnableUNICODE(1);
	/* Init joystick subsystem on all platforms */
#ifdef USE_JOYSTICK
...
#ifdef USE_JOYSTICK
	/* Auto-enable joystick mode on iPhone (accelerometer) */
	if (joystick_available) {
		set_joy_mode();
		printf("Joystick mode enabled (accelerometer)\n");
	}
#endif
	Uint32 flags = 0;
		...
		case SDL_MOUSEMOTION:
			{
				...
				Uint16 keys[2] = {0, 0};
				int nkeys = 0;
				if (tx < 80) keys[nkeys++] = SDLK_LEFT;
				else if (tx > 239) keys[nkeys++] = SDLK_RIGHT;
				if (ty < 67) keys[nkeys++] = SDLK_UP;
				else if (ty > 133) keys[nkeys++] = SDLK_DOWN;
				if (nkeys == 0) keys[nkeys++] = SDLK_LSHIFT;
				{
					int k;
#ifdef TOUCH_DEBUG
					const char *evname = "???";
					if (event.type == SDL_MOUSEBUTTONDOWN) evname = "MouseDown";
					else if (event.type == SDL_MOUSEBUTTONUP) evname = "MouseUp";
					else if (event.type == SDL_MOUSEMOTION) evname = "MouseMotion";
					fprintf(stderr, "idle: %s (%d,%d) -> keys[%d] = ", evname, tx, ty, nkeys);
					for (k = 0; k < nkeys; k++) fprintf(stderr, "0x%X ", keys[k]);
					fprintf(stderr, "\n");
#endif
					int down = (event.type == SDL_MOUSEBUTTONDOWN || event.type == SDL_MOUSEMOTION);
					if (!down) {
					for (k = 0; k < nkeys; k++) key_states[keys[k]] = 0;
				} else {
					key_states[SDLK_LEFT] = 0; key_states[SDLK_RIGHT] = 0;
					key_states[SDLK_UP] = 0; key_states[SDLK_DOWN] = 0;
					key_states[SDLK_LSHIFT] = 0;
					for (k = 0; k < nkeys; k++) key_states[keys[k]] = 1;
					/* In joystick mode, any touch is also an action press */
					if (is_joyst_mode) key_states[SDLK_LSHIFT] = 1;
				}
			}
		}
	break;
